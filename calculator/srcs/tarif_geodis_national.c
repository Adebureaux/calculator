...
double	tarif(int zoneMessagerie, int poids, int mode, int corse)
{
	...
	if (poids > 1000)
		printf("Poids incorrect : le poids ne doit pas depasser 1000 kg.\n");
	if (corse)
	{
		if (!mode)
			prixBase = tableMessagerieCorse[zoneMessagerie][tranchePoids];
		else
			prixBase = tableMessagerieExpressCorse[zoneMessagerie][tranchePoids];
		}
	else
	{
		if (!mode)
